 ...
 * <li> <em>filename</em> is the name of an problem specification input file of
 * predicates as documented in the assignment spec and specificed by the
 * {@link cpsc433.SisyphusPredicates} interface.
 * <li> <em>maxtime</em> is and integer time in seconds specifying the time limit
 * for which the program is not allowed to exceed.
 * </ul>
 * 
 * If the program finds a solution, it MUST output a solution to a file with the
 * same name (including extension) suffixed with ".out". ie: if the input
 * filename is "test.txt", the output filename must be "test.txt.out". It the
 * program finds a solution and successfully writes out the solution to the
 * output file, it must exit with an exit status of 0.
 * <p>
 ...
 * </ul>
 * 
 * If the program is interrupted (eg: ^C) it will nevertheless write out the
 * best solution so far to the output file.
 * <p>
 * 
 * <p>
...
public class Test {
	 ...
	 */
	static final int SUCCESS = 0, FAILED_READ = -1, NO_SOLUTION_FOUND = -2,
			WRITE_FAILED = -3;
	...
	final static int S_SUCCESS = 0, S_PROCESS_FORCEBLY_TERMINATED = 1,
			S_NO_OUTPUT = -1, S_UNPARSEABLE_OUTPUT = -2,
			S_NO_OUTPUT_FORCED = -3, S_UNPARSEABLE_OUTPUT_FORCED = -4,
			S_BAD_PROGRAM_FILENAME = -5, S_CLASSS_NOT_FOUND = -6,
			S_CANNOT_OPEN_JAR = -7, S_DATAFILE_MISSING = -8,
	 ...
	 * 
	 * @param runtime
	 *            the length of time (in seconds) the program is allowed to
	...
	static int search(String searchPrg, String datafile, long runtime) {
		...
		if (!prg.exists()) {
			...
			try {
				...
				ZipEntry entry = zip.getEntry(searchPackage + "/" + searchClass
						+ ".class");
				if (entry == null) {
					println("Couldn't find class " + searchPackage + "/"
							+ searchClass + ", trying sisyphusI/SisyphusI...");
					searchPackage = "sisyphusI";
					entry = zip.getEntry(searchPackage + "/" + searchClass
							+ ".class");
					if (entry == null) {
						println("class " + searchPackage + "/" + searchClass
								+ ".class not found in \"" + searchPrg + "\".");
						return S_CLASSS_NOT_FOUND;
					}
				}
			} catch (Exception ex) {
				println("cannot open jar file \"" + searchPrg + "\": "
						+ ex.toString());
		...
		Process proc;
		String command = "java -classpath " + searchPrg + " " + searchPackage
				+ "." + searchClass + " " + datafile + " " + runtime;
		synchronized (command) {
			try {
				...
				ProcessBuilder pb = new ProcessBuilder("java", "-classpath",
						searchPrg, searchPackage + "." + searchClass, datafile,
				...
				long limit = start
						+ max(runtime + 2000,
								new Double(runtime * 1.3).longValue());
				...
				while (System.currentTimeMillis() < limit) {
					...
					try { // check to see if the process is still running -- if
							// it's stopped, break;
						proc.exitValue();
						break;
				...
				try {
					println("Subprocess Exit code = " + proc.exitValue()
							+ " at " + (System.currentTimeMillis() - start)
							+ "ms");
					return S_SUCCESS;
				} catch (IllegalThreadStateException ex2) {
					println("Process failed to terminate, forcably terminated"
							+ " at " + (System.currentTimeMillis() - start)
							+ "ms");
					proc.destroy();
	...
	/**
	 * The main method that reads the command line arguments and executes the
	 * search algorithm to find a solution, which it writes out. Iff there are
	 * no arguments on the command line, it goes into "command mode" as a
	 * convenience.
	 * 
	 * @param args
	...
	public static void main(String[] args) {
		...
		if (args.length >= 2) {
			...
			if (args.length < 3) {
				printSynopsis();
				println("No run time given; assuming run time of " + runtime
						+ " seconds");
			} else {
			...
			evaluate(args[0]);
		} else { // go into "command mode" if there's nothing on the command
					// line
	...
	protected static int evaluate(String searchPrg, String datafile,
			long runtime) {
		String outfilename = makeOutfilename(datafile);
		int ret = search(searchPrg, datafile, runtime);
		if (ret >= 0) {
			if (!new File(outfilename).exists()) {
				println("search program did not generate expected output file \""
						+ outfilename + "\".");
				return (ret == S_PROCESS_FORCEBLY_TERMINATED) ? S_NO_OUTPUT_FORCED
						: S_NO_OUTPUT;
			} else {
				...
				return (ret == S_PROCESS_FORCEBLY_TERMINATED) ? S_UNPARSEABLE_OUTPUT_FORCED
						: S_UNPARSEABLE_OUTPUT;
			}
		} else {
	...
	protected static void evaluate(String searchPrg) {
		...
		for (int i = 0, max = tests.length; i < max; i++) {
			...
			println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n"
					...
					+ "s) >>>>>>>>>>>>>>>>>\n"
					+ ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			tests[i].result = evaluate(searchPrg, tests[i].datafile,
					tests[i].timelimit);
			println("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n"
					+ "<<<<<<<<<<<<<<<<< test "
					+ i
		...
		for (int i = 0, max = tests.length; i < max; i++) {
			...
			println(t.datafile
					+ pad(t.datafile, 30)
					+ res
					+ pad(res, t.result < 0 ? 50 : 12)
					+ (t.result < 0 ? "" : (t.complete ? "  " : "in")
							+ "complete "
							+ (t.complete == t.completable ? " OK  " : "wrong")
							+ " " + (t.solved ? "  " : "un") + "solved "
							+ (t.solved == t.solvable ? " OK  " : "wrong")
							+ " " + pad(String.valueOf(t.utility), 6)
							+ t.utility) + pad(String.valueOf(secLimit), 6)
					+ secLimit + pad(String.valueOf(secs), 6) + secs);
		}
		println("--------------------------------------------------------------------------------------------");
	}
	...
	/**
	 * Implment "command mode": repeatedly read lines of predictes from
	 ...
	 * 
	 * @param in
	 *            A String representing the input filename
	 * @return an appropriate output filename (which is the input filename +
	 *         ".out")
	 */
